...
export default function AdminAuditClient() {
  ...
  const {
    ...
    applyFirstPage,
    appendPage,
  } = useCursorPagination<AdminAuditRow>();
  ...
  async function handleExportJsonl() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/admin-actions/export?${qs.toString()}`,
        filename: "admin-audit-export.jsonl",
      });
    } finally {
      setDownloading(null);
    }
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/admin-actions/export/csv?${qs.toString()}`,
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-lg font-semibold">Admin Audit</h1>
