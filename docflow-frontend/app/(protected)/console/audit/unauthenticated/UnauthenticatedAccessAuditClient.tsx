...
export default function UnauthenticatedAccessAuditClient() {
  ...
  const {
    ...
    applyFirstPage,
    appendPage,
  } = useCursorPagination<UnauthenticatedAccessAuditRow>();
  ...
  async function handleQuery() {
    ...
    try {
      ...
      const data =
        await apiFetch<UnauthenticatedAccessAuditCursorPageDTO>(
          `/api/audit/unauthenticated-access?${buildParams().toString()}`
        );
  ...
  async function handleLoadMore() {
    ...
    try {
      const data =
        await apiFetch<UnauthenticatedAccessAuditCursorPageDTO>(
          `/api/audit/unauthenticated-access?${buildParams(
            nextCursorTimestamp,
            nextCursorId
          ).toString()}`
        );
  ...
  const handleVerify = createVerifyHandler(
    "/api/audit/unauthenticated-access",
    from,
    to,
    setVerifyResult,
    setVerifying
  ...
  async function handleExportJsonl() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/unauthenticated-access/export?${qs.toString()}`,
        filename: "unauthenticated-access-export.jsonl",
      });
    } finally {
      setDownloading(null);
    }
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/unauthenticated-access/export/csv?${qs.toString()}`,
        filename: "unauthenticated-access-export.csv",
      });
    } finally {
      setDownloading(null);
    }
