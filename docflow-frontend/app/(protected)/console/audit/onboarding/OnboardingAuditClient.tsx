...
export default function OnboardingAuditClient() {
  ...
  const {
    ...
    applyFirstPage,
    appendPage,
  } = useCursorPagination<OnboardingAuditRow>();
  ...
  function buildParams(cursorTs?: string, cursorId?: string) {
    ...
    if (inviteId.trim()) qs.set("inviteId", inviteId.trim());
  ...
  async function handleExportJsonl() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/onboarding/export?${qs.toString()}`,
        filename: "onboarding-audit-export.jsonl",
      });
    } finally {
      setDownloading(null);
    }
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      await downloadAuditFile({
        path: `/api/audit/onboarding/export/csv?${qs.toString()}`,
        filename: "onboarding-audit-export.csv",
      });
    } finally {
      setDownloading(null);
    }
            ...
            <table className="min-w-full text-xs">
              <thead>
                <tr>
                  ...
                  <th className="p-2 border-b">invite</th>
