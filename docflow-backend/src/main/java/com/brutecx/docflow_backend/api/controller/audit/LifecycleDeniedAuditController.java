...
public class LifecycleDeniedAuditController {
    ...
    public ResponseEntity<LifecycleDeniedAuditCursorPageDTO> query(
        ...
        return ResponseEntity.ok(
                queryService.query(from, to, correlationId, subjectId, cursorTimestamp, cursorId, size)
        );
    }
    ...
    public void exportCsv(
        ...
        response.setCharacterEncoding("UTF-8");
        response.setHeader(HttpHeaders.CACHE_CONTROL, "no-store");
        queryService.streamForensicExportCsv(response, from, to, correlationId, subjectId);
    }
}
